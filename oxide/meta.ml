...
(* uninitialize pi if not already done *)
let var_env_uninit (gamma : var_env) (_ : ty) (pi : place) : var_env tc =
  let* ty_pi = var_env_lookup gamma pi
  in if is_uninit ty_pi then gamma |> succ
